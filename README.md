...
If they are separate files, then ensure they are included in app.yml under the site: configuration
		...
		<?php
		include(sfConfig::get(‘sf_root_dir’) . "/config/pages.yml”); 
		include(sfConfig::get(‘sf_root_dir’) . "/config/listings.yml”);
		?>
...
Enable and set listing RSS settings in listing_types (only required item is rss_enabled)
...
To set up a listing with the new class you will need to add new lines into the configuration for the listing type.  Any items left undefined will use the default classes set in the listingManager (see the manager for more information).
